...
try:  # Permite execução direta via `python execute_nova_act.py`
    ...
    from .beforeeach_setup import (
        apply_beforeeach_setup,
        get_init_script_bodies,
        infer_beforeeach_file,
    )
...
class NovaActExecutor:
    ...
    def execute(self, test_case: TestCaseDefinition) -> NovaActResult:
        ...
        try:
            ...
            beforeeach_file = self.beforeeach_test_file or infer_beforeeach_file(
                test_case.name
            )
            init_script_bodies: List[str] = (
                get_init_script_bodies(beforeeach_file)
                if beforeeach_file
                else []
            )

            # Verifica se há setup de beforeEach para este teste
            has_beforeeach_setup = bool(init_script_bodies)
            ...
            with NovaAct(
                ...
                if has_beforeeach_setup and beforeeach_file:
                    self.logger.info(
                        "🔧 Aplicando beforeEach setup de %s", beforeeach_file
                    )
                    apply_beforeeach_setup(
                        nova.page, beforeeach_file, init_script_bodies
                    )
